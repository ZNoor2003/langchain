...
# CHANGE 3: Delete the JSON schemas. Tools now live inside the prompt as plain as text.
...
1. NEVER guess or assume any product price. You MUST call the get_product_price tool to look up the real price of the product.\n
...
def run_agent(question:str):
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        # Stop token prevents the LLM from genrating its own Observation -
        ...
        # Manually replicate what "stop" sequence would have done:
        # cut off anything the model hallucinated after its own "Observation:"
        if "\nObservation" in output:
            output = output.split("\nObservation")[0].rstrip()
        ...
        # CHANGE 6: Parse tool calls from row text withregex - fragile if LLM doesn't follow format.
